...
const checkAndGetUserLikedVideos = async (req, res, next) => {
   try {
      ...
      res.status(500).json({
         success: false,
         message: "could not retreive user Liked Videos",
...
const toggleLikedVideo = async (req, res) => {
   try {
      ...
      if (isVideoPresent) {
         userLikedVideos.videos = userLikedVideos.videos.filter(
      ...
      res.status(500).json({
         success: false,
         message: "could not update user liked videos",
         errorMessage: error.message,
      });
   }
};
